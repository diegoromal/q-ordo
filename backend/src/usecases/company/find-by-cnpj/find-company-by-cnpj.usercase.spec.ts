...
describe('Usecases > Company > FindCompanyByCnpjUsecase', () => {
  ...
  it('should return a company when cnpj exists', async () => {
    ...
    aGateway.findByCnpj = jest.fn().mockResolvedValue(aCompany);

    const usecase = new FindCompanyByCnpjUsecase(aGateway);

    const output = await usecase.execute({ cnpj: '12.345.678/0001-90' });
  ...
  it('should throw when company is not found', async () => {
    ...
    await expect(
      usecase.execute({ cnpj: '12.345.678/0001-90' }),
